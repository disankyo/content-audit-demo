...
发布动态
  ...
  驳回                  通过 / 疑似           失败（重试耗尽）
    │                      │                      │
 biz_status=3         manual_audit_queue     留在队列等待人工介入
...
> **两张队列表都是临时调度数据**，只保存「还没干完的活」，干完即删：
> - 人审队列：只有「待领取 / 已领取」两态，审核提交后删除；结论在 `manual_audit_result`，痕迹在 `manual_audit_log`
> - 机审队列：只有「待处理 / 处理中 / 失败待介入」三态，出结论后删除；
>   **失败任务是故意保留的**——它没有结论可查，删了就彻底丢了，必须留在队列里重试、耗尽后等人工介入
